...
export const handleUpdateProfile = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const { firstName, lastName, password, targetUserId } = req.body;

        const isAdmin = currentUser?.role === "Administrator";

        const userIdToUpdate = isAdmin && currentUser ? targetUserId : currentUser?.userId;

        await updateProfile(userIdToUpdate, {
            firstName: firstName,
            lastName: lastName,
            password: password,
        })
...
export const handleDeleteUser = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if (!userId) {
            res.status(400).json({ message: `No id ${userId} found` })
        }
        await deleteUser(userId)
        res.status(200).json({ message: "Successsfull Deleted User" });
        return;
    } catch (error) {
